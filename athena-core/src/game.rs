...
impl Game {
    ...
    pub fn execute_move(&mut self, mv: Move) {
        match mv {
            ...
            Move::EnPassant { from, to } => {
                ...
                assert!(
                    self.board.get_piece_on_square(to).is_none(),
    ...
    pub(crate) fn get_available_moves(&self) -> Vec<Move> {
        ...
        pieces
            .iter()
            .map(|(sq, pc)| match pc {
                Piece::Rook { .. } => self.moves_from_mask_and_starting_square(
                    &ATTACK_TABLES.get_attack_pattern_rook(*sq, self.board.occupancy),
                    *sq,
                ),
                Piece::Pawn => self.get_pawn_moves(*sq),
                Piece::King { .. } => self.get_king_moves(*sq),
                Piece::Knight => self.moves_from_mask_and_starting_square(
                    &ATTACK_TABLES.get_attack_pattern_knight(*sq),
                    *sq,
                ),
                Piece::Queen => self.moves_from_mask_and_starting_square(
                    &ATTACK_TABLES.get_attack_pattern_queen(*sq, self.board.occupancy),
                    *sq,
                ),
                Piece::Bishop => self.moves_from_mask_and_starting_square(
                    &ATTACK_TABLES.get_attack_pattern_bishop(*sq, self.board.occupancy),
                    *sq,
                ),
            })
            .flatten()
    ...
    fn moves_from_mask_and_starting_square(
        &self,
    ...
    fn get_king_moves(&self, from: Square) -> Vec<Move> {
        let (piece, color) = self
            ...
            .expect("from must contain a piece");
    ...
    fn get_pawn_moves(&self, from: Square) -> Vec<Move> {
        ...
        if let Ok(forward) = from.move_on_rank(heading) {
            ...
            if self.board.get_piece_on_square(forward).is_none() {
                ...
                if (from.get_rank() == Rank::Two && *color == Color::White)
                    || (from.get_rank() == Rank::Seven && *color == Color::Black)
                {
                    if self
                        .board
            ...
            if self.previous_move().is_some_and(|m| {
                ...
                match self
                    ...
                    .get_delta_file(from)
                {
                    1 => available_moves.push(Move::en_pasante(
                        from,
                        forward.move_on_file(1).expect("checked"),
                    )),
                    -1 => available_moves.push(Move::en_pasante(
                        from,
                        forward.move_on_file(-1).expect("checked"),
