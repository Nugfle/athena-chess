...
mod move_generation_tests {
    ...
    use crate::game::board::square::{Square, A1, A4, A7, B1, B2, B4, B6, C1, C3, C4, C5, D1, D2, D3, D4, D5, D6, D7, D8, E1, E2, E3, E4, E5, E6, E7, E8, F2, F4, F5, F6, G1, G3, G4, G5, G7, G8, H1, H4, H8};
    ...
    fn test_sliding_pieces_rook_moves() {
        ...
        for expected_square in expected_squares {
            assert!(moves.iter().any(|mv| mv.get_to() == expected_square), 
                   "Rook should be able to move to {}", expected_square);
        }
    }
    ...
    fn test_sliding_pieces_bishop_moves() {
        ...
        for expected_square in expected_squares {
            assert!(moves.iter().any(|mv| mv.get_to() == expected_square), 
                   "Bishop should be able to move to {}", expected_square);
        }
    }
    ...
    fn test_knight_moves() {
        ...
        for expected_square in expected_squares {
            assert!(moves.iter().any(|mv| mv.get_to() == expected_square), 
                   "Knight should be able to move to {}", expected_square);
        }
    }
    ...
    fn test_en_passant_moves() {
        ...
        let en_passant_move = moves.iter().find(|mv| {
            matches!(mv, Move::EnPassant { from: E5, to: D6 })
        });
    ...
    fn test_pawn_promotions() {
        ...
        let promotion_moves: Vec<_> = moves.iter().filter(|mv| {
            mv.get_from() == E7 && mv.get_to() == E8 && mv.is_promotion()
        }).collect();
        
        assert_eq!(promotion_moves.len(), 4, "Should have 4 promotion options");
        
        // Check that all promotion pieces are available
        let promotion_pieces: Vec<_> = promotion_moves.iter()
            .map(|mv| mv.get_promotion().unwrap())
            .collect();
    ...
    fn test_pawn_promotion_captures() {
        ...
        let promotion_captures: Vec<_> = moves.iter().filter(|mv| {
            mv.get_from() == E7 && mv.get_to() == D8 && mv.is_promotion()
        }).collect();
        ...
        for mv in promotion_captures {
            assert!(matches!(mv, Move::PromotionCapture { captured: Piece::Rook { .. }, .. }));
        }
        
        // Should also have regular promotions
        let regular_promotions: Vec<_> = moves.iter().filter(|mv| {
            mv.get_from() == E7 && mv.get_to() == E8 && mv.is_promotion()
        }).collect();
    ...
    fn test_mixed_piece_scenario() {
        ...
        println!("Total moves: {}, Normal: {}, Captures: {}", 
                moves.len(), normal_moves, capture_moves);
    }
    ...
    fn test_piece_specific_move_counts() {
        ...
        place_piece(&mut game, Piece::Pawn, Color::White, E2);
        game.turn = Color::White;
        assert_eq!(game.get_available_moves().len(), 2, "Pawn should have 2 moves from starting position");
